...
# The MNE reference CSVs only exist on the maintainer's own machine (they're
# generated by rest_filter_helpers.ipynb, not shipped with the package), so
# any test that needs them for exact numerical comparison guards itself with
# skip_if_not(has_ref, ...) below. Everything else (validation/structural
# tests) just needs *some* valid 1-channel signal, so eeg1ch falls back to a
# synthetic one when the reference data isn't available - e.g. in CI - rather
# than failing the whole file at load time.
has_ref    <- dir.exists(REF)
raw_signal <- if (has_ref) {
  load_ref("raw_signal.csv")
} else {
  set.seed(42)
  rnorm(1024)
}
...
test_that(".firwin_kernel BP1 (1-40 Hz, hamming) matches MNE reference kernel", {
  skip_if_not(has_ref, "MNE reference data not available")
