...
test_that(".overlap_add_filter attenuates stop-band content", {
  ...
  # A zero-phase FIR of length N needs (N-1)/2 samples of real signal on each
  # side of every output point; within that reach of either edge, .overlap_add_
  # filter() is convolving against reflect_limited's mirrored (not real) data,
  # so the residual there depends on the signal's phase at that edge - trimming
  # only the start (as this test previously did) leaves it dependent on the
  # phase at the *end*, which is what made it fail intermittently. Trim both.
  reach   <- (length(h) - 1L) %/% 2L
  y_mid   <- y[(reach + 1L):(length(y) - reach)]
  ref_mid <- sin(2 * pi * 10 * t[(reach + 1L):(length(t) - reach)])
...
test_that("eeg_bandpass preserves bads and montage", {
  # eeg_bandpass() reconstructs its return value via new_eeg(); regression
  # test for a bug where bads/montage were dropped because the call site
  # didn't pass them through (new_eeg() defaults both when omitted).
  eeg_state <- eeg1ch
  eeg_state$bads    <- "CH1"
  eeg_state$montage <- create_montage()
...
test_that("eeg_notch preserves bads and montage", {
  # eeg_notch() reconstructs its return value via new_eeg(); regression test
  # for a bug where bads/montage were dropped because the call site didn't
  # pass them through (new_eeg() defaults both when omitted).
  eeg_state <- eeg1ch
  eeg_state$bads    <- "CH1"
  eeg_state$montage <- create_montage()
...
test_that("bandpass then notch preserves bads and montage through the chain", {
  eeg_state <- eeg1ch
  eeg_state$bads    <- "CH1"
  eeg_state$montage <- create_montage()
